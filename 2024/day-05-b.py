...
def modify_update(update: list[int], rules: list[Tuple[int, int]]) -> bool:
    for page in update:
        for rule in rules:
            if page == rule[0]:
                if rule[1] in update:
                    if update.index(rule[1]) < update.index(page):
                        new_index = update.index(page)
                        old_index = update.index(rule[1])
                        update[new_index] = rule[1]
                        update[old_index] = page
                        return False
            if page == rule[1]:
                if rule[0] in update:
                    if update.index(rule[0]) > update.index(page):
                        new_index = update.index(page)
                        old_index = update.index(rule[0])
                        update[new_index] = rule[0]
                        update[old_index] = page
                        return False
    return True
...
def fix_updates(updates: list[list[int]], rules: list[Tuple[int, int]]) -> int:
    count = 0
    for update in updates:
        fix_update(update, rules)
        count += update[int((len(update) - 1)/2)]
    return count
...
if __name__ == "__main__":
    ...
    count = fix_updates(invalid_updates, rules)
    print(f"Total sum of the middle of correct updates is {count}")
